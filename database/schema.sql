...
CREATE TABLE Online_Orders (
    ...
    -- Payment details (recorded when paid)
    payment_method ENUM('cash','card') NULL,
    card_type ENUM('visa','mastercard','amex','discover') NULL,
    card_last4 CHAR(4) NULL,

    CONSTRAINT chk_online_card_last4_digits CHECK (card_last4 IS NULL OR card_last4 REGEXP '^[0-9]{4}$'),
    CONSTRAINT chk_online_payment_card_rules CHECK (
        payment_method IS NULL
        OR (payment_method = 'cash' AND card_type IS NULL AND card_last4 IS NULL)
        OR (payment_method = 'card' AND card_type IS NOT NULL AND card_last4 IS NOT NULL)
    ),
...
-- When an online order is confirmed, automatically create a kitchen ticket
